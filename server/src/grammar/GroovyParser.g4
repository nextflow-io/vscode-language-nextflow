...
localVariableDeclaration
    :   { !GroovyParser.isInvalidLocalVariableDeclaration(this._input) }?
...
commandExpression
    :   expression
        (
            { !GroovyParser.isFollowingArgumentsOrClosure($expression.ctx) }?
            argumentList
        |
 ...
 */
pathExpression returns [number | undefined t]
    :   (
            primary
        |
            // if 'static' followed by DOT, we can treat them as identifiers, e.g. static.unused = { -> }
            { this._input.LT(2).type == GroovyParser.DOT }?
            STATIC
        ) (pathElement { $t = $pathElement.t; })*
    ;

pathElement returns [number | undefined t]
    :   nls
        (
